...
# Pure NumPy-based CNN Inference — Zero TensorFlow/TFLite dependency
...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div class='metric-card'>
            ...
            <div class='metric-value-amber'>CNN · Pure NumPy</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# ──────────────────────────────────────────────
# PURE NUMPY CNN INFERENCE PIPELINE
# ──────────────────────────────────────────────
@st.cache_resource(show_spinner=False)
def load_model_weights():
    """Load the pre-extracted CNN weights from the NPZ archive once, then cache."""
    return np.load("wildfire_weights.npz")
...
def conv2d_np(x, W, b):
    ...
    patches = np.lib.stride_tricks.as_strided(
        x, shape=(oH, oW, kH, kW, x.shape[2]),
        strides=(s[0], s[1], s[0], s[1], s[2])
    )
    return patches.reshape(oH, oW, -1) @ W.reshape(-1, N) + b

def maxpool_np(x, size=2):
    H, W, C = x.shape
    H2, W2 = H // size, W // size
    return x[:H2*size, :W2*size, :].reshape(H2, size, W2, size, C).max(axis=(1, 3))
...
if uploaded_file is not None:
    ...
    with col_result:
        ...
        with st.spinner("⏳ Running orbital pixel analysis matrix..."):
            ...
            # Load model weights from NumPy NPZ
            weights = load_model_weights()

            # Pre-process image to (150, 150, 3)
            img_resized = image.resize((150, 150))
            img_array   = np.array(img_resized, dtype=np.float32) / 255.0

            # Run pure NumPy CNN forward pass
            x = img_array
            x = relu(conv2d_np(x, weights["conv1_W"], weights["conv1_b"])); x = maxpool_np(x)
            x = relu(conv2d_np(x, weights["conv2_W"], weights["conv2_b"])); x = maxpool_np(x)
            x = relu(conv2d_np(x, weights["conv3_W"], weights["conv3_b"])); x = maxpool_np(x)
            x = x.flatten()
            x = relu(x @ weights["fc_W"].astype(np.float32) + weights["fc_b"])
            fire_prob = float(sigmoid(x @ weights["out_W"].astype(np.float32) + weights["out_b"])[0])
